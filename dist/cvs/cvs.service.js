...
let CvsService = class CvsService {
    ...
    async createInitialCv(userId, createCvDto) {
        const savedCv = await this.dataSource.transaction(async (manager) => {
            ...
            const cv = await cvRepository.save(cvRepository.create({
                ...
                sourceType: createCvDto.sourceType === database_enums_1.CvSourceType.IMPROVED
            ...
            const version = await cvVersionRepository.save(cvVersionRepository.create({
                ...
                stylePreset: createCvDto.stylePreset ?? database_enums_1.CvStylePreset.ATS,
                isCurrent: true,
    ...
    async createImprovedVersion(userId, cvId, createImprovedCvVersionDto) {
        ...
        await this.dataSource.transaction(async (manager) => {
            ...
            const nextSummaryText = createImprovedCvVersionDto.summaryText?.trim() ??
            ...
            const newVersion = await cvVersionRepository.save(cvVersionRepository.create({
                ...
                stylePreset: createImprovedCvVersionDto.stylePreset ??
                    sourceVersion.stylePreset ??
                    database_enums_1.CvStylePreset.ATS,
            ...
            await cvRepository.update(sourceCv.id, {
                ...
                targetRole: createImprovedCvVersionDto.targetRole.trim(),
                sourceType: createImprovedCvVersionDto.resultSourceType ??
                    (sourceCv.sourceType === database_enums_1.CvSourceType.CREATED
            ...
            if (sourceVersion.personalDetail) {
                await cvPersonalDetailsRepository.save(cvPersonalDetailsRepository.create({
                    ...
                    professionalSummary: nextSummaryText ??
                        sourceVersion.personalDetail.professionalSummary,
                }));
            }
            const nextWorkExperiences = createImprovedCvVersionDto.workExperiences?.length
            ...
            if (nextWorkExperiences.length) {
                await cvWorkExperiencesRepository.save(nextWorkExperiences.map((workExperience, index) => cvWorkExperiencesRepository.create({
                    cvVersionId: newVersion.id,
                    companyName: workExperience.companyName.trim(),
                    jobTitle: workExperience.jobTitle.trim(),
                    periodLabel: workExperience.periodLabel.trim(),
            ...
            const nextEducationEntries = createImprovedCvVersionDto.educationEntries?.length
            ...
            if (nextEducationEntries.length) {
                await cvEducationEntriesRepository.save(nextEducationEntries.map((educationEntry, index) => cvEducationEntriesRepository.create({
                    cvVersionId: newVersion.id,
                    institutionName: educationEntry.institutionName.trim(),
                    degreeTitle: educationEntry.degreeTitle.trim(),
                    periodLabel: educationEntry.periodLabel.trim(),
    ...
    async createManualEditedVersion(userId, cvId, updateManualCvVersionDto) {
        ...
        await this.dataSource.transaction(async (manager) => {
            ...
            const nextVersionNumber = (latestVersion?.versionNumber ?? 0) + 1;
            const nextTargetRole = updateManualCvVersionDto.targetRole?.trim() ?? sourceVersion.targetRole;
            const nextSummaryText = updateManualCvVersionDto.summaryText?.trim() ?? sourceVersion.summaryText;
            const nextSkillsText = updateManualCvVersionDto.skillsText?.trim() ??
                this.stringifySkillList(updateManualCvVersionDto.skills) ??
                sourceVersion.skillsText;
            const newVersion = await cvVersionRepository.save(cvVersionRepository.create({
                cvId: sourceCv.id,
                versionNumber: nextVersionNumber,
                versionType: database_enums_1.CvVersionType.MANUAL_EDIT,
                targetRole: nextTargetRole,
                jobDescription: updateManualCvVersionDto.jobDescription?.trim() ??
                    sourceVersion.jobDescription,
                summaryText: nextSummaryText,
                skillsText: nextSkillsText,
                stylePreset: updateManualCvVersionDto.stylePreset ??
                    sourceVersion.stylePreset ??
                    database_enums_1.CvStylePreset.ATS,
                generatedFileUrl: updateManualCvVersionDto.generatedFileUrl?.trim() ?? null,
                generatedFileFormat: updateManualCvVersionDto.generatedFileFormat ?? null,
                isCurrent: true,
                createdByProcess: database_enums_1.CreatedByProcess.MANUAL,
            }));
            await cvRepository.update(sourceCv.id, {
                ...
                currentVersionId: newVersion.id,
                targetRole: nextTargetRole,
                sourceType: sourceCv.sourceType === database_enums_1.CvSourceType.IMPROVED
                    ? database_enums_1.CvSourceType.MIXED
                    : sourceCv.sourceType,
            });
            ...
            await cvPersonalDetailsRepository.save(cvPersonalDetailsRepository.create({
                ...
                fullName: updateManualCvVersionDto.personalDetails?.fullName?.trim() ??
                    ...
                    '',
                email: updateManualCvVersionDto.personalDetails?.email?.trim().toLowerCase() ??
            ...
            if (updateManualCvVersionDto.workExperiences) {
                if (updateManualCvVersionDto.workExperiences.length) {
                    await cvWorkExperiencesRepository.save(updateManualCvVersionDto.workExperiences.map((workExperience, index) => cvWorkExperiencesRepository.create({
                        cvVersionId: newVersion.id,
                        companyName: workExperience.companyName.trim(),
                        jobTitle: workExperience.jobTitle.trim(),
                        periodLabel: workExperience.periodLabel.trim(),
            ...
            if (updateManualCvVersionDto.educationEntries) {
                if (updateManualCvVersionDto.educationEntries.length) {
                    await cvEducationEntriesRepository.save(updateManualCvVersionDto.educationEntries.map((educationEntry, index) => cvEducationEntriesRepository.create({
                        cvVersionId: newVersion.id,
                        institutionName: educationEntry.institutionName.trim(),
                        degreeTitle: educationEntry.degreeTitle.trim(),
                        periodLabel: educationEntry.periodLabel.trim(),
            ...
            if (updateManualCvVersionDto.skills) {
                await this.attachSkillsToVersion(newVersion.id, updateManualCvVersionDto.skills, skillsRepository, cvVersionSkillsRepository);
            }
            else if (sourceVersion.versionSkills.length) {
                await cvVersionSkillsRepository.save(sourceVersion.versionSkills.map((versionSkill) => cvVersionSkillsRepository.create({
                    cvVersionId: newVersion.id,
